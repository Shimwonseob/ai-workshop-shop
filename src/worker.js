...
async function productIntro(request, env) {
  ...
  const messages = [
    { role: "system", content: "Write a calm, factual English introduction for the food product described by the Korean source text. Return exactly 2 or 3 complete English sentences, with a period at the end of every sentence. Return only the introduction sentences as one paragraph. Use very simple wording and concrete nouns and verbs; avoid decorative or promotional adjectives. Do not write a title, label, heading, note, preface, translation marker, product-name list, markdown, or words such as Translated, Translation, English, Introduction, Product name, Short description, Description, or Note. Use only facts explicitly present in the source. Never invent origin, ingredients, certification, health benefits, taste, aroma, texture, quality, quantity, or marketing claims. Do not add words such as Korean-style, succulent, crunchy, tender, satisfying, filling, convenient, delicious, premium, fresh, healthy, or flavorful unless the exact fact is stated in the source." },
